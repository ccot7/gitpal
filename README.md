# gitpal - A terminal git companion for senior engineers

Answers git questions instantly from a built-in reference database. 


Translate human language into git commands.
...
Git is powerful - but hard to recall under pressure.

gitpal lets you describe intent in plain English and shows the exact command instantly - with zero guesswork.

---

## Features - what makes gitpal different

- **Zero-latency local lookup** — **no API, no delay**. Fuzzy keyword search over a comprhensive git reference set
- **AI fallback** — **only when needed, not always-on**. Activates automatically when local confidence is low, or on demand
- **Repo-aware AI context** — Your actual branch, status, recent log, and in-progress merge/rebase state are injected into every AI query automatically
...
```sh
git clone git@github.com:ccot7/gitpal.git
cd gitpal
...
gitpal/
  src/
    ...
    gitref.h    — reference database (tldr github pages /common git entries, compiled into binary)
  ...
  tools/
    ...
    tldr/git-pages.md   — bundled (basic not the complete one, use tldr_to_gitref.py for full list) tldr git reference (43 subcommands)
...
Comprehensive coverage (~147 tests):
- command matching
- edge cases
- verbose mode
- repo vs non-repo behavior
